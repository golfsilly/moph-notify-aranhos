...
import { RentIptAllService } from "@/services/rent-ipt/rent-ipt-all.service";
...
export function getCronStatus() {
  ...
  return { "rent-ipt-all": rentIptAllStatus };
}
...
export function startRentIptAllCron() {
  startCron(
    "rent-ipt-all",
    "0 18 * * 1-5", // ทุกวันจันทร์ - ศุกร์ เวลา 18:00 น.
    async () => {
      const result = await RentIptAllService.triggerReport();

      console.log(
        `✅ ส่งรายงานสำเร็จ | Staff: ${result.staff.length} | Intern: ${result.intern.length}`,
      );
    },
  );
}
